#!/usr/bin/env python3
"""Strict native raster and advance invariants for G and its four derivatives."""
from pathlib import Path
from PIL import Image,ImageChops,ImageOps,ImageDraw,ImageFont
from fontTools.ttLib import TTFont
import json,hashlib,argparse
ROOT=Path(__file__).resolve().parents[1];BASE=ROOT/'references/upper-g-baseline';OUT=ROOT/'proofs/upper-g'
...
for weight in weights:
    for size in sizes:
        ...
        for a,b in zip(old['records'],new['records']):
            ...
            ch=b['character'];rows.append({'weight':weight,'size':size,'text':ch,'group':'base' if ch=='G' else 'derivatives' if ch in cfg['affected'] else 'controls',
...
assert changed_gvar==['G'],changed_gvar
...
report={'fontSHA256':sha(fontpath),'beforeFontSHA256':sha(baseline),'holdout':args.holdout,'allReferencesIdentical':True,'allAdvanceModelsIdentical':True,
        ...
        'baseByWeight':{str(w):stats([r for r in rows if r['weight']==w and r['group']=='base']) for w in weights},
...
if not args.holdout:
    sheet=Image.new('RGB',(1200,1040),'white');draw=ImageDraw.Draw(sheet);font=ImageFont.load_default(size=18)
    for col,title in enumerate(('BEFORE / 0.800','CURRENT / 0.810','SYSTEM REFERENCE')):draw.text((col*400+25,20),title,font=font,fill='#444444')
    for row,w in enumerate((100,400,900)):
        ds=[json.loads((OUT/f'{stage}-{w}-128/comparison.json').read_text())['records'][0] for stage in ('before','after')]
        ...
        for col,im in enumerate(ims):
            crop=im.crop(box);sheet.paste(crop,(col*400+(400-crop.width)//2,65+row*310))
        draw.text((25,340+row*310),f'G / {w} / 128 pt / native 2x',font=font,fill='#555555')
    draw.text((25,1000),'Same origin and baseline; no image registration or error tolerance.',font=font,fill='#444444');sheet.save(OUT/'progress.png')
