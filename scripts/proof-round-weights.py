#!/usr/bin/env python3
"""Before/after native verification for the three-weight round weights calibration."""
from pathlib import Path
from PIL import Image,ImageOps,ImageChops,ImageDraw,ImageFont
import json,hashlib
ROOT=Path(__file__).resolve().parents[1];folder=ROOT/'proofs/round-weights'
...
for weight in (100,400,900):
    for size in (16,32,64,128):
        ...
        assert [r['character'] for r in before['records']]==[r['character'] for r in after['records']]==list('oOоО')
...
report={'fontSHA256':sha,'scope':'4 round glyphs, three weights, four native sizes; no full-font or browser acceptance',
...
for col,title in enumerate(('BEFORE / 0.660','CURRENT / 0.670','SYSTEM REFERENCE')):
