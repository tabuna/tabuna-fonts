#!/usr/bin/env python3
"""Strict before/after proof for seven recalibrated polygons and 28 controls."""
from pathlib import Path
from PIL import Image,ImageOps,ImageChops,ImageDraw,ImageFont
import json,hashlib
ROOT=Path(__file__).resolve().parents[1];folder=ROOT/'proofs/latin-rectangles'
sha=hashlib.sha256((ROOT/'dist/TabunaSansVariable.ttf').read_bytes()).hexdigest()
base_sha=hashlib.sha256((ROOT/'references/latin-rectangles-baseline/TabunaSans-before.ttf').read_bytes()).hexdigest()
rows=[]
for weight in (100,400,900):
    for size in (16,32,64,128):
        ...
        assert [r['character'] for r in before['records']]==[r['character'] for r in after['records']]==list('IHEFLЕІПпШшЦцЩщТтНнЬьЪъЫыPРDBВвoOоО')
...
report={'fontSHA256':sha,'beforeFontSHA256':base_sha,'scope':'7 recalibrated polygon glyphs and 28 controls, 3 weights, 4 native sizes; no full-font or browser acceptance',
        'allReferencesIdentical':True,'total':len(rows),
        'newGlyphsByWeight':{str(w):stats([r for r in rows if r['weight']==w and r['character'] in 'IHEFLЕІ']) for w in (100,400,900)},
        'controls':stats([r for r in rows if r['character'] not in 'IHEFLЕІ']),
...
for col,title in enumerate(('BEFORE / 0.680','CURRENT / 0.690','SYSTEM REFERENCE')):
    draw.text((col*400+25,22),title,font=label,fill='#444444')
for row,weight in enumerate((100,400,900)):
    paths=[folder/f'before-{weight}-128/0045-tabuna.png',folder/f'after-{weight}-128/0045-tabuna.png',folder/f'after-{weight}-128/0045-system.png']
