#!/usr/bin/env python3
"""Summarize native before/after shoulder-glyph renders without score inflation."""
from pathlib import Path
from PIL import Image,ImageOps,ImageChops,ImageDraw,ImageFont
import json,hashlib
ROOT=Path(__file__).resolve().parents[1];folder=ROOT/'proofs/shoulders'
...
report={'fontSHA256':font_hash,'scope':'Three Regular shoulder glyphs at four native sizes; not full-font acceptance',
...
sheet=Image.new('RGB',(900,835),'white');draw=ImageDraw.Draw(sheet)
label=ImageFont.load_default(size=18)
for col,title in enumerate(('BEFORE / 0.610','CURRENT / 0.620','SYSTEM REFERENCE')):
    draw.text((col*300+25,22),title,font=label,fill='#444444')
for row,code in enumerate(('006E','0068','006D')):
...
draw.text((25,798),'Widths match. Nonzero pixel differences remain.',font=label,fill='#444444')
