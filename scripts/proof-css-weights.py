#!/usr/bin/env python3
"""Before/after native verification for the three-weight CSS axis-weight calibration."""
from pathlib import Path
from PIL import Image,ImageOps,ImageChops,ImageDraw,ImageFont
import json,hashlib
ROOT=Path(__file__).resolve().parents[1];folder=ROOT/'proofs/css-weights'
...
for weight in (100,400,900):
    for size in (16,32,64,128):
        ...
        assert [r['character'] for r in before['records']]==[r['character'] for r in after['records']]==list('ПпШшЦцЩщТтНнЬьЪъЫыPРDBВвoOоО')
...
report={'fontSHA256':sha,'scope':'28 calibrated glyphs, three weights, four native sizes; no full-font or browser acceptance',
...
for col,title in enumerate(('BEFORE / 0.670','CURRENT / 0.680','SYSTEM REFERENCE')):
    draw.text((col*400+25,22),title,font=label,fill='#444444')
for row,weight in enumerate((100,400,900)):
    paths=[folder/f'before-{weight}-128/041E-tabuna.png',folder/f'after-{weight}-128/041E-tabuna.png',folder/f'after-{weight}-128/041E-system.png']
    ...
    draw.text((25,290+row*265),f'U+041E / weight {weight} / 128 pt / native 2x',font=label,fill='#555555')
