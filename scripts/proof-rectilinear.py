#!/usr/bin/env python3
"""Before/after native verification for the three-weight Cyrillic calibration."""
from pathlib import Path
from PIL import Image,ImageOps,ImageChops,ImageDraw,ImageFont
import json,hashlib
ROOT=Path(__file__).resolve().parents[1];folder=ROOT/'proofs/rectilinear'
...
report={'fontSHA256':sha,'scope':'12 rectilinear Cyrillic glyphs, three weights, four native sizes; no full-font or browser acceptance',
...
for col,title in enumerate(('BEFORE / 0.620','CURRENT / 0.630','SYSTEM REFERENCE')):
    draw.text((col*400+25,22),title,font=label,fill='#444444')
for row,weight in enumerate((100,400,900)):
    paths=[folder/f'before-{weight}-128/0449-tabuna.png',folder/f'after-{weight}-128/0449-tabuna.png',folder/f'after-{weight}-128/0449-system.png']
    ...
    draw.text((25,290+row*265),f'U+0449 / weight {weight} / 128 pt / native 2x',font=label,fill='#555555')
