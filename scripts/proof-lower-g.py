#!/usr/bin/env python3
"""Strict before/after proof for one original g, four derivatives, and 51 controls."""
from pathlib import Path
from PIL import Image,ImageOps,ImageChops,ImageDraw,ImageFont
import json,hashlib
ROOT=Path(__file__).resolve().parents[1];folder=ROOT/'proofs/lower-g'
sha=hashlib.sha256((ROOT/'dist/TabunaSansVariable.ttf').read_bytes()).hexdigest()
base_sha=hashlib.sha256((ROOT/'references/lower-g-baseline/TabunaSans-before.ttf').read_bytes()).hexdigest()
rows=[]
for weight in (100,400,900):
    for size in (16,32,64,128):
        ...
        assert [r['character'] for r in before['records']]==[r['character'] for r in after['records']]==list('gĝğġģbdpqрIHEFLЕІPРDBВвoOоОПпШшЦцЩщТтНнЬьЪъЫыnhmaаeеcCсС')
        for a,b in zip(before['records'],after['records']):
            ...
            assert a['tabuna']['advance']==b['tabuna']['advance'],(b['character'],weight,size,'Advance changed')
...
report={'fontSHA256':sha,'beforeFontSHA256':base_sha,'scope':'g and four derivatives with 51 controls, 3 weights, 4 native sizes; no full-font or browser acceptance',
        'allReferencesIdentical':True,'total':len(rows),
        'newGlyphsByWeight':{str(w):stats([r for r in rows if r['weight']==w and r['character'] in 'gĝğġģ']) for w in (100,400,900)},
        'baseByWeight':{str(w):stats([r for r in rows if r['weight']==w and r['character']=='g']) for w in (100,400,900)},
        'derivativesByCharacter':{ch:stats([r for r in rows if r['character']==ch]) for ch in 'ĝğġģ'},
        'controls':stats([r for r in rows if r['character'] not in 'gĝğġģ']),
...
for col,title in enumerate(('BEFORE / 0.780','CURRENT / 0.790','SYSTEM REFERENCE')):
    draw.text((col*400+25,22),title,font=label,fill='#444444')
for row,weight in enumerate((100,400,900)):
    paths=[folder/f'before-{weight}-128/0067-tabuna.png',folder/f'after-{weight}-128/0067-tabuna.png',folder/f'after-{weight}-128/0067-system.png']
    ...
    draw.text((25,290+row*265),f'g / weight {weight} / 128 pt / native 2x',font=label,fill='#555555')
