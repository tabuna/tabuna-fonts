#!/usr/bin/env python3
"""Before/after native verification for the three-weight double bowl calibration."""
from pathlib import Path
from PIL import Image,ImageOps,ImageChops,ImageDraw,ImageFont
import json,hashlib
ROOT=Path(__file__).resolve().parents[1];folder=ROOT/'proofs/double-bowls'
...
report={'fontSHA256':sha,'scope':'3 double bowl glyphs, three weights, four native sizes; no full-font or browser acceptance',
...
for col,title in enumerate(('BEFORE / 0.650','CURRENT / 0.660','SYSTEM REFERENCE')):
    draw.text((col*400+25,22),title,font=label,fill='#444444')
for row,weight in enumerate((100,400,900)):
    paths=[folder/f'before-{weight}-128/0412-tabuna.png',folder/f'after-{weight}-128/0412-tabuna.png',folder/f'after-{weight}-128/0412-system.png']
    ...
    draw.text((25,290+row*265),f'U+0412 / weight {weight} / 128 pt / native 2x',font=label,fill='#555555')
