#!/usr/bin/env python3
"""Strict before/after proof for three recalibrated shoulders and 35 controls."""
from pathlib import Path
from PIL import Image,ImageOps,ImageChops,ImageDraw,ImageFont
import json,hashlib
ROOT=Path(__file__).resolve().parents[1];folder=ROOT/'proofs/shoulder-weights'
sha=hashlib.sha256((ROOT/'dist/TabunaSansVariable.ttf').read_bytes()).hexdigest()
base_sha=hashlib.sha256((ROOT/'references/shoulder-weights-baseline/TabunaSans-before.ttf').read_bytes()).hexdigest()
rows=[]
for weight in (100,400,900):
    for size in (16,32,64,128):
        ...
        assert [r['character'] for r in before['records']]==[r['character'] for r in after['records']]==list('nhmIHEFLЕІПпШшЦцЩщТтНнЬьЪъЫыPРDBВвoOоО')
...
report={'fontSHA256':sha,'beforeFontSHA256':base_sha,'scope':'3 recalibrated shoulder glyphs and 35 controls, 3 weights, 4 native sizes; no full-font or browser acceptance',
        'allReferencesIdentical':True,'total':len(rows),
        'newGlyphsByWeight':{str(w):stats([r for r in rows if r['weight']==w and r['character'] in 'nhm']) for w in (100,400,900)},
        'controls':stats([r for r in rows if r['character'] not in 'nhm']),
...
for col,title in enumerate(('BEFORE / 0.700','CURRENT / 0.710','SYSTEM REFERENCE')):
    draw.text((col*400+25,22),title,font=label,fill='#444444')
for row,weight in enumerate((100,400,900)):
    paths=[folder/f'before-{weight}-128/006D-tabuna.png',folder/f'after-{weight}-128/006D-tabuna.png',folder/f'after-{weight}-128/006D-system.png']
    ...
    draw.text((25,290+row*265),f'm / weight {weight} / 128 pt / native 2x',font=label,fill='#555555')
