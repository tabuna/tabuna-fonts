#!/usr/bin/env python3
"""Strict before/after proof for two original lower-case a glyphs and 38 controls."""
from pathlib import Path
from PIL import Image,ImageOps,ImageChops,ImageDraw,ImageFont
import json,hashlib
ROOT=Path(__file__).resolve().parents[1];folder=ROOT/'proofs/lower-a'
sha=hashlib.sha256((ROOT/'dist/TabunaSansVariable.ttf').read_bytes()).hexdigest()
base_sha=hashlib.sha256((ROOT/'references/lower-a-baseline/TabunaSans-before.ttf').read_bytes()).hexdigest()
rows=[]
for weight in (100,400,900):
    for size in (16,32,64,128):
        ...
        assert [r['character'] for r in before['records']]==[r['character'] for r in after['records']]==list('aаnhmIHEFLЕІПпШшЦцЩщТтНнЬьЪъЫыPРDBВвoOоО')
...
report={'fontSHA256':sha,'beforeFontSHA256':base_sha,'scope':'2 original lower-case a glyphs and 38 controls, 3 weights, 4 native sizes; no full-font or browser acceptance',
        'allReferencesIdentical':True,'total':len(rows),
        'newGlyphsByWeight':{str(w):stats([r for r in rows if r['weight']==w and r['character'] in 'aа']) for w in (100,400,900)},
        'controls':stats([r for r in rows if r['character'] not in 'aа']),
...
for col,title in enumerate(('BEFORE / 0.720','CURRENT / 0.730','SYSTEM REFERENCE')):
