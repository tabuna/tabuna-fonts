#!/usr/bin/env python3
"""Strict before/after proof for four original open-round glyphs and 42 controls."""
from pathlib import Path
from PIL import Image,ImageOps,ImageChops,ImageDraw,ImageFont
import json,hashlib
ROOT=Path(__file__).resolve().parents[1];folder=ROOT/'proofs/open-rounds'
sha=hashlib.sha256((ROOT/'dist/TabunaSansVariable.ttf').read_bytes()).hexdigest()
base_sha=hashlib.sha256((ROOT/'references/open-rounds-baseline/TabunaSans-before.ttf').read_bytes()).hexdigest()
rows=[]
for weight in (100,400,900):
    for size in (16,32,64,128):
        ...
        assert [r['character'] for r in before['records']]==[r['character'] for r in after['records']]==list('cCсСeеaаnhmIHEFLЕІПпШшЦцЩщТтНнЬьЪъЫыPРDBВвoOоО')
...
report={'fontSHA256':sha,'beforeFontSHA256':base_sha,'scope':'4 original open-round glyphs and 42 controls, 3 weights, 4 native sizes; no full-font or browser acceptance',
        'allReferencesIdentical':True,'total':len(rows),
        'newGlyphsByWeight':{str(w):stats([r for r in rows if r['weight']==w and r['character'] in 'cCсС']) for w in (100,400,900)},
        'controls':stats([r for r in rows if r['character'] not in 'cCсС']),
...
for col,title in enumerate(('BEFORE / 0.740','CURRENT / 0.750','SYSTEM REFERENCE')):
    draw.text((col*400+25,22),title,font=label,fill='#444444')
for row,weight in enumerate((100,400,900)):
    paths=[folder/f'before-{weight}-128/0063-tabuna.png',folder/f'after-{weight}-128/0063-tabuna.png',folder/f'after-{weight}-128/0063-system.png']
    ...
    draw.text((25,290+row*265),f'c / weight {weight} / 128 pt / native 2x',font=label,fill='#555555')
