#!/usr/bin/env python3
"""Strict native proof for the authored M/W polygons."""
...
for weight in weights:
 for size in sizes:
  prefix='holdout-' if args.holdout else '';folders=[OUT/f'{prefix}{stage}-{weight}-{size}' for stage in ('before','after')];old,new=[json.loads((p/'comparison.json').read_text()) for p in folders]
...
changed=[n for n in before.getGlyphOrder() if before['gvar'].variations[n]!=after['gvar'].variations[n]];assert changed==['M','W'],changed
