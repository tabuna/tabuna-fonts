...
import json,hashlib
ROOT=Path(__file__).resolve().parents[1];OUT=ROOT/'proofs/stem-bowls'
sha=lambda p:hashlib.sha256(p.read_bytes()).hexdigest()
font=ROOT/'dist/TabunaSansVariable.ttf';baseline=ROOT/'references/stem-bowls-baseline/TabunaSans-before.ttf'
rows=[]
for weight in (200,300,500,600,700,800):
    for size in (14,20,24,48,96):
        ...
        assert [r['character'] for r in old['records']]==[r['character'] for r in new['records']]==list('bdpqр')
...
assert changed==['b','d','p','q','uni0440'],changed
report={'fontSHA256':sha(font),'beforeFontSHA256':sha(baseline),'scope':'Five glyphs × six unfitted weights × five unfitted sizes; native pixel and scalar comparison, no tolerance',
        ...
        'all':stats(rows),'byWeight':{str(w):stats([r for r in rows if r['weight']==w]) for w in (200,300,500,600,700,800)},'rows':rows}
