#!/usr/bin/env python3
"""Strict native raster and advance invariants for U/u and their canonical derivatives."""
from pathlib import Path
from PIL import Image,ImageChops,ImageOps,ImageDraw,ImageFont
from fontTools.ttLib import TTFont
import json,hashlib,argparse
ROOT=Path(__file__).resolve().parents[1];BASE=ROOT/'references/u-bowls-baseline';OUT=ROOT/'proofs/u-bowls'
...
for weight in weights:
    for size in sizes:
        ...
        for a,b in zip(old['records'],new['records']):
            ...
            ch=b['character'];rows.append({'weight':weight,'size':size,'text':ch,'group':'base' if ch in 'Uu' else 'derivatives' if ch in cfg['affected'] else 'controls',
...
assert changed_gvar==['U','u'],changed_gvar
...
report={'fontSHA256':sha(fontpath),'beforeFontSHA256':sha(baseline),'holdout':args.holdout,'allReferencesIdentical':True,'allAdvanceModelsIdentical':True,
        ...
        'baseByWeight':{ch:{str(w):stats([r for r in rows if r['weight']==w and r['text']==ch]) for w in weights} for ch in 'Uu'},
...
if not args.holdout:
    ...
    for col,title in enumerate(('BEFORE / 0.810','CURRENT / 0.820','SYSTEM REFERENCE')):draw.text((col*400+25,20),title,font=font,fill='#444444')
    for row,(ch,w) in enumerate((('U',400),('u',100),('u',900))):
