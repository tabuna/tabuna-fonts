"""Separate outer silhouette and hole measurements before fitting the shared bowl/tail model for 6."""
...
def fit(font,construction,initial):
    gs=font.getGlyphSet();path=pathops.Path();gs[font.getBestCmap()[54]].draw(path.getPen())
    ...
    cs=[(1-(np.array(c)-lo)/(hi-lo)).tolist() for c in flat.contours]
    def area(c):
        ...
        for role in roles:
            for key,v in p[role].items():
                if key=='handles':
                    ...
                    else:low.append(max(-.01,v-.08));high.append(min(1.01,v+.08))
        def params(v):
            q=copy.deepcopy(p)
...
def main():
    ...
    sys.path.insert(0,str(args.workspace/'scripts'));spec=importlib.util.spec_from_file_location('nine',args.workspace/'scripts/nine_bowl.py');m=importlib.util.module_from_spec(spec);spec.loader.exec_module(m)
    f=TTFont('/System/Library/Fonts/SFNS.ttf');o=subset.Options();o.layout_features=[];s=subset.Subsetter(options=o);s.populate(text='6');s.subset(f)
    out=args.workspace/'sources/six-bowl.json';seed=json.loads(args.seed.read_text())['weights'];d=json.loads(out.read_text()) if args.resume and out.exists() else dict(weights={},measurements=[])
