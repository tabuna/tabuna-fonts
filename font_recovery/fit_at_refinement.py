...
from at_spiral import construction,asymmetric_bowl
...
def fit_counter(hole):
    pts=np.asarray(hole);lo=pts.min(0);hi=pts.max(0);normalized=((pts-lo)/(hi-lo)).tolist()
    target=shape_features([normalized],[0,0,1,1],count=121,nonzero=True)
    def residual(v):
        p=Flatten(None);asymmetric_bowl([0,0,1,1],v[:8].reshape(4,2),v[8:]).replay(p)
        return shape_features(p.contours,[0,0,1,1],count=121,nonzero=True)-target
    fit=least_squares(residual,[.55]*8+[.5]*4,bounds=([.05]*8+[.2]*4,[.98]*8+[.8]*4),max_nfev=160,diff_step=1e-4,x_scale='jac',ftol=1e-9,xtol=1e-9,gtol=1e-9)
    return {'bounds':[np.r_[lo,hi].tolist()],'outerHandles':fit.x[:8].reshape(4,2).tolist(),'extrema':fit.x[8:].tolist()},[float(np.linalg.norm(fit.fun))]
...
def main():
    ...
    for w in [100,400,900]:
      for label,opt in [('text',17),('display',28)]:
        ...
        gs=f.getGlyphSet(location=dict(wght=w,opsz=opt,wdth=100,GRAD=400));flat=Flatten(gs);gs[f.getBestCmap()[64]].draw(flat);assert len(flat.contours)==2;allpts=np.concatenate(flat.contours);lo=allpts.min(0);hi=allpts.max(0);cs=[((np.array(c)-lo)/(hi-lo)).tolist() for c in flat.contours];hole=min(cs,key=lambda c:np.prod(np.ptp(np.array(c),axis=0)));counter,e=fit_counter(hole);p=copy.deepcopy(json.load(open(a.seed))['weights'][str(w)][label]) if a.seed else seed(w);p['bounds']=[0,0,1,1];p['counter_bounds']=counter['bounds'][0];p['counter_handles']=counter['outerHandles'];p['counter_extrema']=counter['extrema'];p.setdefault('bowl_extrema',[.5]*4);t={100:.035,400:.07,900:.13}[w];cb=p['counter_bounds'];p['bowl_bounds']=p.get('bowl_bounds',[cb[0]-t,cb[1]-t,cb[2]+t*.5,cb[3]+t])
        ...
        for role in ['outer','inner']:
            q=p['spiral'][role]
            q.setdefault('tip_slope',p['spiral']['tip_slope'])
        ...
        def visit(obj,path=()):
            if isinstance(obj,dict):
                for k,v in obj.items():
                    if not path and k in ['bounds','counter_bounds','counter_handles','counter_extrema']:continue
        ...
        def residual(v):
            q=Flatten(None);construction(params(v)).replay(q);return shape_features(q.contours,[0,0,1,1],count=91,nonzero=True)-target
        fit=least_squares(residual,vals,bounds=(lows,highs),max_nfev=160,diff_step=1e-4,ftol=1e-9,xtol=1e-9,gtol=1e-9,x_scale='jac');q=params(fit.x);q['bounds']=(np.r_[lo,hi]*1000/f['head'].unitsPerEm).tolist();profiles[label]=q;row=dict(weight=w,optical=opt,residual=float(np.linalg.norm(fit.fun)),converged=bool(fit.success),evaluations=fit.nfev,counter_residual=e[0]);data['measurements'].append(row);a.out.parent.mkdir(parents=True,exist_ok=True);a.out.write_text(json.dumps(data,indent=2)+'\n');print(row,flush=True)
