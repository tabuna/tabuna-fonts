"""Original alphabet skeletons, separated from scripts/design.py.

Only project-authored line, ribbon and bowl constructions are retained.
Reference-coordinate branches, refined overrides and environment pilots are
excluded. These skeletons complete coverage while family fitting continues.
They are not evidence of raster acceptance. See data/alphabet-provenance.json.
"""
from copy import copy
from font_recovery.drawing import Drawing
from fontTools.pens.boundsPen import BoundsPen


def draw_skeleton(pen, character, parameters):
    """Size an authored skeleton using scalar metrics, without reference nodes."""
    design = Design(parameters['weight'], 28)
    design.s *= parameters['stroke_scale']
    drawing, _ = (design.latin(character) if character.isascii()
                  else design.cyrillic(character))
    bounds = BoundsPen(None)
    drawing.replay(bounds)
    left, bottom, right, top = bounds.bounds
    target_left, target_bottom, target_right, target_top = parameters['bbox']
    sx = (target_right - target_left) / (right - left)
    sy = (target_top - target_bottom) / (top - bottom)
    drawing.replay(pen, (sx, 0, 0, sy, target_left - sx * left,
                        target_bottom - sy * bottom))


def draw_accent(pen, parameters):
    """Two dots or a curved breve, described by an envelope and thickness."""
    left, bottom, right, top = parameters['accent_bbox']
    drawing = Drawing()
    if parameters['accent'] == 'dieresis':
        diameter = min(top - bottom, (right - left) / 3)
        drawing.ellipse(left, bottom, left + diameter, top)
        drawing.ellipse(right - diameter, bottom, right, top)
    else:
        width = right - left
        height = top - bottom
        thickness = parameters['accent_stroke']
        drawing.stroke((left + thickness / 2, top), [
            ((left + thickness / 2, bottom - height * .15),
             (right - thickness / 2, bottom - height * .15),
             (right - thickness / 2, top)),
        ], thickness)
        # Preserve the measured accent envelope after ribbon offsetting.
        bounds = BoundsPen(None)
        drawing.replay(bounds)
        a, b, c, d = bounds.bounds
        sx, sy = width / (c - a), height / (d - b)
        drawing.replay(pen, (sx, 0, 0, sy, left - sx * a, bottom - sy * b))
        return
    drawing.replay(pen)
...
class Design:
    ...
    def latin(self, ch):
        ...
        if ch in 'Oo0':
            ...
            d.line((s * 0.5, y * 0.39), (w - s * 0.45, y - s * 0.2), s * 0.93)
            d.line((s * 0.5, y * 0.43), (w - s * 0.45, s * 0.2), s * 0.98)
        elif ch == 'L':
            ...
            d.line((s * 0.47, h), (w * 0.5, h * 0.06), s * 0.95)
            path((w - s * 0.47, h), (w * 0.37, -80), ((w * 0.3, -186), (w * 0.19, -220 + sy / 2), (0,
                -204 + sy / 2)), sw=s * 0.94)
        elif ch == '1':
            stem(w - s)
    ...
    def cyrillic(self, ch):
        ...
        if ch == 'б':
            ...
            # A curved left leg joins the roof tangentially. Explicit inner
            # and outer edges avoid offset cusps at the roof's sharp turn.
            roof = left + (right - left) * .2
            d.outline((left, 0), [
                (left, sy),
                ((roof, sy), (roof, H * .58), (roof, H)),
                (right, H), (right, 0), (right - s, 0),
                (right - s, H - sy), (roof + s, H - sy),
                ((roof + s, H * .54), (roof + s, 0), (left, 0)),
            ])
            if c == 'Д':
                bar(0)
                stem(0, -130, sy)
            ...
            for side in (-1, 1):
                ...
                d.line((center, H * 0.44), (center + side * (w * 0.5 - s * 0.4), s * 0.17), s * 0.98)
        elif c == 'З':
            ...
            u_right_top_inset = 0
            d.line((s * 0.45, H), (w * 0.48, H * 0.28), s * 0.95)
            path((w - s * (0.45 + u_right_top_inset), H), (w * 0.4, H * 0.15), ((w * 0.25, sy / 2),
                (w * 0.16, sy / 2), (s * 0.5, sy / 2)), sw=s * 0.97)
        elif c == 'Ф':
