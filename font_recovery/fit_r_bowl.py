"""Measure R stem, counter and leg; fit shared bowl to scalar sections."""
...
def main():
 ...
 for w in [100,400,900]:
  for label,opt in [('text',17),('display',28)]:
   gs=f.getGlyphSet(location=dict(wght=w,opsz=opt,wdth=100,GRAD=400));flat=Flatten(gs);gs[f.getBestCmap()[82]].draw(flat);assert len(flat.contours)==2;v=np.concatenate(flat.contours);lo=v.min(0);hi=v.max(0);cs=[((np.array(c)-lo)/(hi-lo)).tolist() for c in flat.contours]
