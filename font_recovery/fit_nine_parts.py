"""Separate outer silhouette and hole measurements before fitting the authored 9."""
...
def fit(font,construction,initial):
    gs=font.getGlyphSet();path=pathops.Path();gs[font.getBestCmap()[57]].draw(path.getPen())
...
def main():
    ...
    f=TTFont('/System/Library/Fonts/SFNS.ttf');o=subset.Options();o.layout_features=[];s=subset.Subsetter(options=o);s.populate(text='9');s.subset(f)
    out=args.workspace/'sources/nine-bowl.json';seed=json.loads(args.seed.read_text())['weights'];d=json.loads(out.read_text()) if args.resume and out.exists() else dict(weights={},measurements=[])
